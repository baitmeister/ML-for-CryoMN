...
The v2 selector always proposes 12 viability/intact screens and resolves
mechanical allocation from completed screening and paired formulation–batch
evidence.

| Phase | Entry condition | Mechanical allocation |
|---|---|---:|
| `screening_only` | Fewer than 8 completed screening rounds | None |
| `mechanics_bootstrap` | Screening entry met; fewer than 8 pairs, 6 formulations, or 2 paired batches | 4 bootstrap tests |
| `mechanics_hybrid` | Hybrid gate met; fewer than 16 pairs, 12 formulations, or 3 paired batches | 2 qLogNEHVI, 1 local, 1 coverage |
| `mechanics_enabled` | Full 16/12/3 gate met | 4 full-mechanics tests plus ordered eligible backups |

A pair requires viability and critical load for the same formulation and batch;
technical replicates aggregate into one pair. Mechanics is performed only on
ranked rows that actually pass intact, with backups promoted in numeric order.
Exact mechanically observed formulations are excluded except for the single
anchor allowed in the second bootstrap batch.

The formulas, anchor rules, repeat exclusions, worksheet contract, fallbacks,
and manual-override semantics are defined in the
[canonical v2 policy](src/08_multi_objective/README.md#evidence-gated-mechanics-transition).
Instructions for changing the gate values are in
[Changing the transition policy](src/08_multi_objective/README.md#changing-the-transition-policy).
