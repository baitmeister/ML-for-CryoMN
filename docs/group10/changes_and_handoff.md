...
| Reference control | Exact recipe exception, readiness, exclusion from production models | Pending reagent/base-medium/replicate settings |
...
| Confirmation | Typed repeated-mechanics role and scheduled reservation | Disabled pending cadence/settings |
...
No shared-model, adaptive-noise, batch-adjustment, or new mechanical-target activation is implemented in the production selector. A later explicit user decision during full mechanical testing is required. No four-unknown exploration cap was introduced.
...
7. Use raw reference monitoring, not normalization. Review the model audit separately during full mechanics.
