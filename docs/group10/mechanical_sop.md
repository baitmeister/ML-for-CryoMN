...
Use the Group 10 worksheet frozen by this branch. For each compressed specimen, provide its `instron_file`, actual `needles_compressed` when known, `replicate_id` (or a separate row that gets an automatic replicate ID), and fresh viability/intact results as appropriate. The ordinary round-update command extracts the endpoint using that proposal's frozen settings. There is no requirement to enter replicate totals in advance.

Leave `critical_axial_load_N_per_needle`, `critical_axial_load_N_total` and the old stiffness field blank for automatic extraction. A supplied force must match the reproducible terminal value within 1e-6 N in its field's units; a mismatched old maximum is rejected. An unlabeled manual force without raw data is not accepted under this new protocol. Do not run the legacy maximum-force sheet-population helper for a Group 10 terminal-force worksheet.
...
Group 9 supplied endpoint-development evidence. Its six raw curves can be reviewed with this extractor in separate output files, but their frozen observations/worksheet remain under the original contract. No automatic historical re-labeling is performed. Observed frontiers and prospective metric pools identify endpoint definitions and never pool old maximum-force and new terminal-force labels. No prospective claim is made for choosing 0.8 mm using Group 9 curves.
